Feature: Show Notebook
  ...
  Background: notes have been added to the database
    ...
    |3  | Quick Sort             | 1          | 1          | Quicksort is an efficient sorting algorithm. Developed by British computer scientist Tony Hoare within 1959 and published within 1961, it is still a commonly used algorithm for sorting. When implemented well, it can be somewhat faster than merge sort and about two or three times faster than heapsort.                                                                                                                                                        |
    |4  | Heap Sort              | 1          | 1          | Heapsort can be thought of as an improved selection sort: like selection sort, heapsort divides its input into a sorted and an unsorted region, and it iteratively shrinks the unsorted region by extracting the largest element from it and inserting it into the sorted region.                                                                                                                                                                                    |
...
Scenario: If i click on a tag I should be taken to the search page for the tag
  ...
  Scenario: On the details should be an add button and edit only if user login and user own this note
    When I am logged in as "example1@gmail.com"
    When I go to the details page for "Sorting Algorithms"
    Then I should see "Add Note"
    And  I should see "Edit Note"
  
  Scenario: On the details page, user should not see add or edit if not login
    When I go to the details page for "Sorting Algorithms"
    Then I should not see "Add Note"
    And  I should not see "Edit Note" 
  
  Scenario: On the details page, user should not see add or edit if user login but not own this note
    When I am logged in as "example1@gmail.com"
    When I go to the details page for "Graph Theory"
    Then I should not see "Add Note"
    And  I should not see "Edit Note"